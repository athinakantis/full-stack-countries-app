describe('Testing favorites', () => {
  ...
  it('Can add countries as favorites', () => {
    cy.visit('/countries')
    cy.intercept('POST', `${Cypress.env('SUPABASE_URL')}/rest/v1/country_favorites*`).as('favoriteRequest');
  ...
  it('Can remove from favorites', () => {
    cy.visit('/favorites')
    cy.intercept('DELETE', `${Cypress.env('SUPABASE_URL')}/rest/v1/country_favorites*`).as('favoriteRequest');
    cy.get('button[data-test-id="favorite-button"]').first().click()
    cy.wait('@favoriteRequest')
